...
@endif<div class="card card-primary">
    <div class="card-header">
        <h3 class="card-title d-flex "> <a href="{{route('admin.role.index')}}"><i class="fas fa-undo-alt"></i></a></h3>
    </div>
    <form action="{{route('admin.role.update',['id'=>$roles->id])}}" method="post">
        @csrf
        <div class="card-body">

            <div class="form-group">
                <label for="exampleInputEmail1">Tên chức vụ</label>
                <input type="text" class="form-control" value="{{$roles->name}}" name="name" id="exampleInputEmail1" placeholder="Nhập tên chức vụ">
                <code> {{ $errors->first('name') }} </code>
            </div>
            <div class="form-group">
                <label for="exampleInputEmail1">Mô tả chức vụ</label>

                <textarea name="desc" class="form-control" id="exampleInputEmail1" cols="30" rows="10" placeholder="Mô tả chức vụ">{{$roles->desc}}</textarea>
